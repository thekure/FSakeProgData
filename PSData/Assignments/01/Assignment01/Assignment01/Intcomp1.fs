...
let rec eval e (env : (string * int) list) : int =
    ...
    | CstI i            -> i
    | Var x             -> lookup env x 
    | Let(list, ebody) -> // let x1 = 5+7     x2 = x1*2       in       x1+x2      end
        List.fold (fun acc (str, expr) -> (str, eval expr env) :: acc) env list |> eval ebody
...
let rec freevars e : string list =
    ...
    | Var x  -> [x]
    | Let(list, ebody) -> 
          List.fold (fun acc (str, expr) -> (union (freevars expr, minus (freevars ebody, [str]))) @ acc ) [] list          
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let(list, ebody) ->
        ...
        | (str, expr) :: xs ->
            let cenv1 = str :: cenv
            TLet(tcomp expr cenv, tcomp (Let(xs, ebody)) cenv1)

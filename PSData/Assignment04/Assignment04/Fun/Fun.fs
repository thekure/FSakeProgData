...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Letfun(f, args, fBody, letBody) -> 
      let fClosure = Closure(f, args, fBody, env)
    ...
    | Call(Var f, eLst) -> 
      ...
      match fClosure with
      | Closure(f, pNames, fBody, fDeclEnv) ->
        if List.length pNames <> List.length eLst then
          failwith "eval Call: argument count mismatch"
        else
          let vLst = List.map (fun e -> Int (eval e env)) eLst
          let evZip = List.zip pNames vLst
          let fBodyEnv = evZip @ (f, fClosure) :: fDeclEnv
          eval fBody fBodyEnv
...
let ex11 = Letfun("f1", ["x"; "y"], Prim("+", Var "x", Var "y"), 
                 Call(Var "f1", [CstI 12; CstI 8]));;
